  ...
  class StrengthChecker
    ...
    def is_strong?(min_entropy = BASE_ENTROPY, use_dictionary = false, min_word_length = 4, extra_dictionary_words = [])
      weak = (EntropyCalculator.calculate(base_password) < min_entropy) ||
             ...
             (QwertyAdjuster.new(base_password).is_weak?(min_entropy))
      if !weak && use_dictionary
        return DictionaryAdjuster.new(base_password).is_strong?(min_entropy,
                    min_word_length,
                    extra_dictionary_words)
      else
        return !weak
    ...
    def calculate_entropy(use_dictionary = false, min_word_length = 4, extra_dictionary_words = [])
      ...
      entropies << DictionaryAdjuster.new(base_password).adjusted_entropy(min_word_length, extra_dictionary_words) if use_dictionary
      entropies.min
    end
  end
end